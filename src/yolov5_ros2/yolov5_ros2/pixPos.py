...
def Cal_n3_2(u,v,y):
    ...
    point_test = uv2world(u,v, mtx, dist, 1.0, R, tvec)
    point_test_2 = uv2world(u,v, mtx, dist, 2.0, R, tvec)

    tmp_x = point_test_2[0, 0] - point_test[0, 0]
    tmp_y = point_test_2[1, 0] - point_test[1, 0]
    tmp_z = point_test_2[2, 0] - point_test[2, 0]
    n2 = (y - (point_test[1, 0])) / tmp_y
    p_w_3D = np.array([[0.0], [0.0], [0.0]])
    p_w_3D[0, 0] = point_test[0, 0] + n2 * tmp_x
    p_w_3D[1, 0] = y
    p_w_3D[2, 0] = point_test[2, 0] + n2 * tmp_z

    return p_w_3D

def getPos(U, V):
    Y = 0.84; #根据坐标系提前确定高程值 
    point_world_D = Cal_n3_2(U,V,Y)
    return point_world_D

if __name__ == "__main__":
    point_world_D = getPos(304.0,423.0)
    print(point_world_D)
